...
export async function upsertCarePlan(
  ...
  const existing = (await client
    .request(
      `CarePlan?patient=Patient/${patient.id}&status=active,on-hold,draft&_count=100`,
      { flat: true }
    )
    .catch(() => [])) as any[];
...
/* communication to patient removed by refactor */
...
export async function createMedicationStatement(
  ...
  const existing = (await client
    .request(
      `MedicationStatement?patient=Patient/${patient.id}&_sort=-dateasserted&_count=20`,
      { flat: true }
    )
    .catch(() => [])) as any[];
...
export async function createAppointment(
  ...
  const future = (await client
    .request(
      `Appointment?actor=${encodeURIComponent(`Practitioner/${practitionerId}`)}&date=ge${encodeURIComponent(startISO)}&date=lt${encodeURIComponent(endISO)}&_sort=date&_count=50"Patient already has a future appointment. Please cancel existing appointment first.");
  }
...
/* inbox function removed by refactor */
    );
    return r || [];
  } catch {
    return [];
...
export async function listPatientMedicationStatements(
  ...
  try {
    ...
    const r = await client.request(
      `MedicationStatement?patient=Patient/${patientId}&effective=ge${dateFilter}&_sort=-effective&_count=100`,
      { flat: true }
    );
    ...
    if (!r || r.length === 0) {
      const allStatements = await client.request(
        `MedicationStatement?patient=Patient/${patientId}&_sort=-effective&_count=50`,
        { flat: true }
      );
...
export async function listCarePlans(patientId: string): Promise<any[]> {
  ...
  try {
    const r = await client.request(
      `CarePlan?patient=Patient/${patientId}&_sort=-_lastUpdated&_count=20`,
      { flat: true }
    );
    return r || [];
  } catch {
...
export async function listServiceRequests(patientId: string): Promise<any[]> {
  ...
  try {
    const r = await client.request(
      `ServiceRequest?patient=Patient/${patientId}&_sort=-authored&_count=20`,
      { flat: true }
    );
    return r || [];
  } catch {
...
export async function listAppointments(patientId: string): Promise<any[]> {
  ...
  try {
    const r = await client.request(
      `Appointment?patient=Patient/${patientId}&_sort=-date&_count=20`,
      { flat: true }
    );
    return r || [];
  } catch {
...
}`)}&date=ge${encodeURIComponent(startISO)}&date=lt${encodeURIComponent(endISO)}&_sort=date&_count=50"Error fetching appointments:", err);
    return [];
  }
}
