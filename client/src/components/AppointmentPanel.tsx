...
import { getClient } from '../fhir';
import { dayBoundsISO, formatLocalTime } from '../utils/tz';

type Props = { practitionerId: string; title?: string };
...
export default function AppointmentsPanel({ practitionerId, title = 'Appointments (today)' }: Props) {
  ...
  const load = React.useCallback(async () => {
    ...
    try {
      ...
      const q = `Appointment?actor=${encodeURIComponent(`Practitioner/${practitionerId}`)}&date=ge${encodeURIComponent(startISO)}&date=lt${encodeURIComponent(endISO)}&_sort=date&_count=50`;
      const bundle: any = await client.request(q, { method: 'GET' });
