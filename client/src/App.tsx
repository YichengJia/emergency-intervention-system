...
<div style={{marginTop: 12}}>
  ...
  const summary: RiskSummary | undefined = useMemo(() => {
    ...
    const risk = riskFromCount(edCount);
  ...
  const createEducation = async (text: string) => {
    if (!client || !patient) return;
    await createCommunication(client, patient, text);
  };
  ...
  return (
    <div style={{ maxWidth: 900, margin: "0 auto", padding: 16 }}>
      ...
      <div style={{ marginTop: 12, fontSize: 12, color: "#666" }}>
        <p>
          This prototype uses synthetic data and writes FHIR resources back to the sandbox for
          auditing (CarePlan, ServiceRequest, Communication).
        </p>
      </div>
    </div>
  );
